...
<c:choose>
    ...
    <c:when test="${userInfo.nickName != forumDTO.name}">
        <div class="noto500 ls_1" style="font-size: 30px; color: #333; padding-bottom: 50px;">
            이용할 수 없는 페이지입니다.
        </div>
        <a href="${contextPath}/forum/" class="h_btn_b icon_b uk-icon" uk-icon="icon:list; ratio:1">목록보기<svg width="20" height="20" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" data-svg="list"><rect x="6" y="4" width="12" height="1"></rect><rect x="6" y="9" width="12" height="1"></rect><rect x="6" y="14" width="12" height="1"></rect><rect x="2" y="4" width="2" height="1"></rect><rect x="2" y="9" width="2" height="1"></rect><rect x="2" y="14" width="2" height="1"></rect></svg></a>
    ...
    <c:otherwise>
<form name="regiform" method="post" action="${contextPath}/forum/modify" onsubmit="return check_Valid();" enctype="multipart/form-data" target="h_blank">
...
<div style="width:100%;">
	...
	<div style="border-top:1px solid #4d4d4d;" class="bbs_reg_table">
		...
		<tbody>
			...
			<input type="hidden" name="page" value="${forumPage.page }">
			<input type="hidden" name="pageNum" value="${forumPage.pageNum }">
			<input type="hidden" name="searchType" value="${forumPage.searchType }">
			<input type="hidden" name="keyword" value="${forumPage.keyword }">
		<tr>
			<th>등록인</th>
			<td class="h_form">
				<input type="text" name="name" value="${forumDTO.name }" readonly disabled>
			</td>
		</tr>
		...
		<tr>
			<th>제목</th>
			<td class="h_form"><input name="title" type="text" value="${forumDTO.title }"></td>
		</tr>
		<tr>
			<td colspan="2" style="padding:10px 0px;">
				<!-- 위지윅 시작 -->
				<!--  에디터 관련 CSS -->
			<textarea id="content" name="content">${forumDTO.content }</textarea>
				<!-- 위지윅끝 -->
			</td>
		</tr>
		</tbody></table>
